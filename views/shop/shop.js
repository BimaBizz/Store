const { createApp } = Vue;
        ...
        createApp({
            data() {
                return {
                    currentTab: window.location.pathname.includes('/tracker') ? 'tracker' : (window.location.pathname.includes('/dashboard') ? 'dashboard' : 'shop'),
                    settings: {},
                    products: [],
                    categories: [],
            ...
            methods: {
                        ...
                        if (data.order.snap_token && typeof snap !== 'undefined') {
                            ...
                            snap.pay(data.order.snap_token, {
                                onSuccess: function(result) {
                                    ...
                                    self.removeVoucher();
                                    window.location.href = finishedBase +
                                        '?order_id=' + encodeURIComponent(result.order_id || orderData.transaction_id) +
                                        '&status_code=' + encodeURIComponent(result.status_code || '200') +
                                        '&transaction_status=' + encodeURIComponent(result.transaction_status || 'settlement');
                                },
                                onPending: function(result) {
                                    ...
                                    window.location.href = finishedBase +
                                        '?order_id=' + encodeURIComponent(result.order_id || orderData.transaction_id) +
                                        '&status_code=' + encodeURIComponent(result.status_code || '201') +
                                        '&transaction_status=' + encodeURIComponent(result.transaction_status || 'pending');
                                },
                                onError: function(result) {
                                    ...
                                    window.location.href = finishedBase +
                                        '?order_id=' + encodeURIComponent(result.order_id || orderData.transaction_id) +
                                        '&status_code=' + encodeURIComponent(result.status_code || '400') +
                                ...
                                onClose: function() {
                                    ...
                                    self.removeVoucher();
                                    window.location.href = finishedBase +
                                        '?order_id=' + encodeURIComponent(orderData.transaction_id) +
                ...
                cancelPayment() {
                    ...
                    if (order) {
                        window.location.href = finishedBase +
                            '?order_id=' + encodeURIComponent(order.transaction_id || '') +
                            '&status_code=201&transaction_status=pending';
                    }
                },
                payPendingOrder(order) {
                    ...
                    if (order.snap_token && typeof snap !== 'undefined') {
                        ...
                        snap.pay(order.snap_token, {
                            onSuccess: function(result) {
                                window.location.href = finishedBase +
                                    '?order_id=' + encodeURIComponent(result.order_id || order.transaction_id) +
                                    '&status_code=' + encodeURIComponent(result.status_code || '200') +
                                    '&transaction_status=' + encodeURIComponent(result.transaction_status || 'settlement');
                            },
                            onPending: function(result) {
                ...
                simulatePaymentSuccess() {
                    ...
                    setTimeout(() => {
                        try {
                            ...
                            this.removeVoucher();
                            window.location.href = finishedBase +
                                '?order_id=' + encodeURIComponent((order && order.transaction_id) || '') +
