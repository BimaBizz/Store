    ...
    <div class="product-detail-grid">
        ...
        <div class="product-detail-media">
            <div class="product-img-wrapper-detail" style="border-radius: 16px; overflow: hidden; background: var(--card-bg); border: 1px solid var(--border-color); box-shadow: var(--shadow-sm); aspect-ratio: 1; display: flex; align-items: center; justify-content: center;">
                <img :src="selectedProduct.image_url || 'https://images.unsplash.com/photo-1509042239860-f550ce710b93?w=800'" :alt="selectedProduct.name" style="width: 100%; height: 100%; object-fit: cover;">
            </div>
        </div>
        ...
        <div class="product-detail-info" style="display: flex; flex-direction: column; gap: 1.5rem;">
            <div>
                ...
                <h1 style="font-family: var(--font-title); font-size: 2.5rem; font-weight: 800; line-height: 1.2; margin-top: 0.25rem; color: #fff;">{{ selectedProduct.name }}</h1>
            ...
            <div style="display: flex; align-items: center; gap: 1.5rem; flex-wrap: wrap;">
                <div v-if="selectedProduct.stock > 0" style="display: flex; align-items: center; border: 1px solid var(--border-color); border-radius: 8px; overflow: hidden; background: rgba(255,255,255,0.03);">
                    <button class="qty-btn" @click="detailQty = Math.max(1, detailQty - 1)" style="padding: 0.75rem 1.25rem; background: transparent; border: none; color: #fff; cursor: pointer; transition: background 0.2s;">-</button>
                    <span style="padding: 0 1rem; font-weight: 600; min-width: 2rem; text-align: center;">{{ detailQty }}</span>
                    <button class="qty-btn" @click="detailQty = Math.min(selectedProduct.stock, detailQty + 1)" style="padding: 0.75rem 1.25rem; background: transparent; border: none; color: #fff; cursor: pointer; transition: background 0.2s;">+</button>
                </div>
